...
- Look for any task with `status: in_progress` in `PLAN/tasks/`
...
- Scan all task files in `PLAN/tasks/` with `status: todo`
...
- **Understand the data model first:** Read relevant Supabase migrations in `supabase/migrations/` and types in `src/types/` to understand the schema before writing code
...
- **Generate `PLAN/pending-push.sh`** -- append the push/PR commands for this task:
  ```
  #!/bin/bash
  ...
  gh pr create --base main --head <branch-name> --title "task(<ID>): <title>" --body "Automated migration task. See PLAN/tasks/<ID>-*.md for details."
  gh pr merge --merge --auto
  ```
- If `pending-push.sh` already exists, **append** the new task's commands (do not overwrite)
- Make it executable: `chmod +x PLAN/pending-push.sh`
...
- **Check data schema** before implementing -- read relevant migrations and `src/types/supabase.ts` to understand tables, columns, RLS policies, and relationships
