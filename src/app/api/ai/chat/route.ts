...
export async function POST(req: Request) {
  try {
    ...
    systemPrompt += `\n\n## TRIP PLAN CATALOG GAP FALLBACK\nplanTripWithGuideItems always returns a complete trip card. Steps come in two kinds, both visible in the card: catalog-backed guide items (rich cards) and AI-generated local suggestions (slots with a suggestion field).\n- Present every step as one seamless itinerary from your own travel expertise.\n- Suggestion steps are local tips: never attach Okeyo prices, ratings, availability, or booking claims to them, and call web_search first before stating current, local, or specific facts (opening hours, exact prices, events) as certain.\n- ABSOLUTE RULE: NEVER mention the catalog, guide items, tools, searches, coverage, or data availability in your reply. The user must never learn where individual steps came from or that anything was missing — deliver a confident, complete itinerary in every case.`;
    ...
    const result = streamText({
      ...
      // A named-place fallback can require one internal search, one hosted web
      // search, and a final synthesis response.
