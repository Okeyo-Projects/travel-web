...
export function BookingProvider({ children }: { children: React.ReactNode }) {
  ...
  const [experience, setExperience] = React.useState<ExperienceDetail | null>(
    null,
  );
  ...
  const stepsOrder: BookingStep[] = React.useMemo(() => {
    if (experience?.type === "lodging") {
      // Mobile order: guests → options (rooms) → dates → promo → review
      return ["guests", "options", "dates", "promo", "review"];
    }
    // Trip: options (departures) → guests → promo → review
  ...
  const value = {
    ...
    openBooking,
    closeBooking,
    setDates: (s: Date | undefined, e: Date | undefined) => {
      setStartDate(s);
      setEndDate(e);
    },
    setGuests,
