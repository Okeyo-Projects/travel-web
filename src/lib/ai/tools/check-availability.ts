...
const checkAvailabilitySchema = z.object({
  ...
  date_to: z
    .string()
    .optional()
    .describe("End date in YYYY-MM-DD format (for lodging)"),
    ...
    try {
      ...
      if (experience.type === "lodging") {
        ...
        const { data: roomTypes } = await db
          ...
          .select(
            "id, room_type, name, capacity_beds, max_persons, price_cents, total_rooms",
          )
        ...
        const availableDepartures = departures?.filter(
          (dep: any) =>
            dep.seats_available > 0 &&
            (!guests || dep.seats_available >= guests),
        );
        ...
        return {
          ...
          departures:
            availableDepartures?.map((dep: any) => ({
              ...
              price_mad: dep.price_override_cents
        ...
        const availableSessions = sessions?.filter(
          (session: any) =>
            session.capacity_available > 0 &&
            (!guests || session.capacity_available >= guests),
        );
        ...
        return {
          ...
          sessions:
            availableSessions?.map((session: any) => ({
              ...
              price_mad: session.price_override_cents
