...
function createSelectRoomTypeSchema(defaultLanguage: SupportedLanguage) {
  return z.object({
...
export function createSelectRoomTypeTool(
...
Use this before createBookingIntent when the user must choose a specific room type.`,
    inputSchema: createSelectRoomTypeSchema(defaultLanguage),
      ...
      try {
        ...
        const filteredByGuests =
          typeof guests === "number"
            ? roomTypes.filter((room) => room.max_persons >= guests)
            : roomTypes;

        const effectiveRooms =
          filteredByGuests.length > 0 ? filteredByGuests : roomTypes;

        const rooms = effectiveRooms.slice(0, max_options).map((room) => {
...
export const selectRoomType = createSelectRoomTypeTool();
