-- =====================================================
-- SIMPLIFY BOOKING QUOTE LOGIC
-- =====================================================
-- This migration updates get_booking_quote to handle
-- auto-apply promotions internally, removing the need
-- for separate client-side logic.
-- =====================================================
...
BEGIN
  ...
  -- Only execute if we have a valid user and price
  IF p_user_id IS NOT NULL AND v_subtotal > 0 THEN
    ...
    IF EXISTS (
      ...
      -- Case 1: Manual Code Provided
      IF p_promotion_code IS NOT NULL AND length(p_promotion_code) > 0 THEN
        ...
        IF FOUND THEN
          IF is_promotion_available(v_promotion.id, p_user_id, v_subtotal, p_experience_id) THEN
            ...
            IF v_discount > 0 THEN
               v_promotion_message := format('Promotion "%s" applied.', v_promotion.name);
            END IF;
          ELSE
            v_promotion_message := format('Promotion "%s" is not valid for this booking.', p_promotion_code);
          END IF;
        ELSE
          v_promotion_message := format('Promotion code "%s" not found.', p_promotion_code);
        END IF;

      -- Case 2: Auto-Apply (No code provided)
      ELSE
        -- Find all eligible auto-apply promotions and pick the best one
        FOR v_promotion IN 
        ...
        LOOP
          ...
          IF is_promotion_available(v_promotion.id, p_user_id, v_subtotal, p_experience_id) THEN
             ...
             IF v_current_discount > v_best_discount THEN
               ...
               v_best_promo_id := v_promotion.id;
               v_best_promo_name := v_promotion.name;
             END IF;
          END IF;
        END LOOP;

        -- Apply the best auto-promotion found
        IF v_best_discount > 0 THEN
          v_discount := v_best_discount;
          v_promotion_message := format('%s applied automatically.', v_best_promo_name);
        END IF;
      END IF;

      -- Apply discount to breakdown if valid
      IF v_discount > 0 THEN
        v_breakdown := v_breakdown || jsonb_build_object(
          'label', CASE 
             WHEN v_best_promo_name IS NOT NULL THEN format('Discount (%s)', v_best_promo_name)
             WHEN v_promotion.name IS NOT NULL THEN format('Discount (%s)', v_promotion.name)
             ELSE 'Discount'
          END,
...
-- Add comment with parameter types to ensure uniqueness
COMMENT ON FUNCTION get_booking_quote(UUID, DATE, DATE, INT, INT, INT, JSONB, UUID, TEXT, UUID) IS
  'Calculate booking quote with server-enforced pricing and auto-apply promotions.';
