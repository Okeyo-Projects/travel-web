-- =====================================================
-- SIMPLIFIED BOOKING QUOTE WITH AUTO-APPLY
-- =====================================================
-- This migration updates get_booking_quote to automatically
-- find and apply the best promotion if none is provided.
-- This removes complex logic from the client.
-- =====================================================
...
CREATE OR REPLACE FUNCTION get_booking_quote(
  ...
  p_rooms TEXT DEFAULT NULL,  -- Changed to TEXT to handle JSON strings from client
...
BEGIN
  ...
  -- 3. Promotion Logic (The New Simplified Part)
  IF p_user_id IS NOT NULL AND v_subtotal > 0 THEN
    ...
    ELSE
      ...
      -- We use a cursor or simple loop. Here we'll loop over potential candidates.
      FOR v_promotion IN 
      ...
      IF v_best_promotion IS NOT NULL THEN
        ...
        -- Add promotion metadata to breakdown for Client UI to use if needed
        -- (Client can look for 'discount_cents' > 0)
      END IF;
    END IF;
  END IF;
...
COMMENT ON FUNCTION get_booking_quote(UUID, DATE, DATE, INT, INT, INT, TEXT, UUID, TEXT, UUID) IS
  'Auto-calculates pricing including automatic application of best available promotions. p_rooms accepts JSON string from client.';
