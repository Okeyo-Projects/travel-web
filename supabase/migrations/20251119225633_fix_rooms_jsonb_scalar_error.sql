-- =====================================================
-- FIX: Cannot get array length of a scalar error
-- =====================================================
-- This migration fixes the "cannot get array length of a scalar"
-- error by ensuring p_rooms parameter accepts TEXT and is
-- properly converted to JSONB before array operations.
-- =====================================================
...
CREATE OR REPLACE FUNCTION get_booking_quote(
  ...
  p_rooms TEXT DEFAULT NULL,  -- Accept TEXT (JSON string) from client
...
DECLARE
  ...
  v_rooms_jsonb JSONB;  -- Converted rooms array
...
BEGIN
  ...
  -- Lodging Calculation
  IF v_experience_type = 'lodging' THEN
    -- Handle p_rooms: convert TEXT (JSON string) to JSONB safely
    IF p_rooms IS NULL OR trim(p_rooms) = '' THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      RETURN QUERY SELECT 0, 0, 0, 0, 0, 'MAD'::TEXT, v_nights, '[]'::JSONB,
                          FALSE, format('Invalid rooms JSON: %s', SQLERRM)::TEXT;
      RETURN;
    END;

    -- Validate it's an array before using array functions
    IF jsonb_typeof(v_rooms_jsonb) != 'array' THEN
      RETURN QUERY SELECT 0, 0, 0, 0, 0, 'MAD'::TEXT, v_nights, '[]'::JSONB,
                          FALSE, 'Rooms must be a JSON array'::TEXT;
      RETURN;
    END IF;

    -- Now safe to use jsonb_array_length
  ...
  ELSIF v_experience_type = 'activity' THEN
    ...
    IF v_metadata IS NOT NULL THEN
      ...
      v_currency := COALESCE(v_metadata->'pricing'->>'currency', v_metadata->>'currency', v_currency);
    END IF;
  ...
  -- 3. Promotion Logic (Auto-apply if no code provided)
  IF p_user_id IS NOT NULL AND v_subtotal > 0 THEN
    ...
    IF p_promotion_code IS NOT NULL AND trim(p_promotion_code) != '' THEN
      ...
      FROM promotions
      WHERE (lower(code) = lower(p_promotion_code) AND is_case_sensitive = false
    ...
    ELSE
      FOR v_promotion IN 
        ...
        WHERE auto_apply = true 
          ...
          AND valid_from <= NOW()
          AND (applicable_experience_ids IS NULL OR p_experience_id = ANY(applicable_experience_ids))
      LOOP
        IF is_promotion_available(v_promotion.id, p_user_id, v_subtotal, p_experience_id) THEN
...
-- Update comment with correct parameter types
COMMENT ON FUNCTION get_booking_quote(UUID, DATE, DATE, INT, INT, INT, TEXT, UUID, TEXT, UUID) IS
  'Calculate booking quote with auto-apply promotions. p_rooms accepts JSON string (TEXT) from client and converts to JSONB internally.';
