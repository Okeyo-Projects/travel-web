-- =====================================================
-- FIX: Remove function overload ambiguity
-- =====================================================
-- This migration drops all existing versions of get_booking_quote
-- and creates a single version with TEXT parameter for p_rooms.
-- This fixes the "Could not choose the best candidate function" error.
-- =====================================================

-- Drop ALL existing versions of get_booking_quote
DROP FUNCTION IF EXISTS get_booking_quote(
  UUID, DATE, DATE, INT, INT, INT, JSONB, UUID, TEXT, UUID
);

DROP FUNCTION IF EXISTS get_booking_quote(
  UUID, DATE, DATE, INT, INT, INT, TEXT, UUID, TEXT, UUID
);

-- Create single version with TEXT parameter (client sends JSON.stringify result)
...
BEGIN
  ...
  IF v_experience_type = 'lodging' THEN
    ...
    LOOP
      ...
      WHERE lrt.id = v_room_type_id AND lrt.experience_id = p_experience_id AND lrt.deleted_at IS NULL;
  ...
  ELSIF v_experience_type = 'activity' THEN
    ...
    IF v_metadata IS NOT NULL THEN
      ...
      -- Extract currency from metadata (no ambiguity here as it's JSONB extraction)
      v_currency := COALESCE(
...
COMMENT ON FUNCTION get_booking_quote(UUID, DATE, DATE, INT, INT, INT, TEXT, UUID, TEXT, UUID) IS
  'Calculate booking quote with automatic promotion application. Accepts p_rooms as TEXT (JSON string from client).';
