-- =====================================================
-- Add city_slug_filter parameter to search_experiences_enhanced
-- This allows filtering by city_slug (FK to cities table)
-- which is more reliable than text-based city_filter matching.
-- =====================================================

DROP FUNCTION IF EXISTS search_experiences_enhanced;
...
BEGIN
  ...
  WITH base_experiences AS (
    ...
    WHERE
      ...
      -- City slug filter (uses FK to cities table)
      AND (
        city_slug_filter IS NULL
        OR LOWER(e.city_slug) = LOWER(city_slug_filter)
      )
      -- City text filter
...
COMMENT ON FUNCTION search_experiences_enhanced IS 'Enhanced search combining semantic search, distance, promotions, and availability checks. Supports city_slug_filter for FK-based city matching.';
